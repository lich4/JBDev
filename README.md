...
https://github.com/user-attachments/assets/56ed7115-a267-4aa5-bb9e-1d4d838d651d
...
## Support Logos Syntax

### Configure Xcode to Auto-compile ".x/.xm" Files

Build Rules - Add:
* Files '*.x' using Script
	* Source files with names matching: "*.x"
	* Custom script(uncheck 'Run once'): `$THEOS/bin/logos.pl -c warnings=error $INPUT_FILE_PATH > $DERIVED_FILE_DIR/$INPUT_FILE_NAME.m`
	* Output Files: `${DERIVED_FILE_DIR}/${INPUT_FILE_NAME}.m`
* Files '*.xm' using Script
	* Source files with names matching: "*.xm"
	* Custom script(uncheck 'Run once'): `$THEOS/bin/logos.pl -c warnings=error $INPUT_FILE_PATH > $DERIVED_FILE_DIR/$INPUT_FILE_NAME.mm`
	* Output Files: `${DERIVED_FILE_DIR}/${INPUT_FILE_NAME}.mm`

### Force Xcode to Treat ".x/.xm" Files as Source Code

* Build Phases - Compile Sources - add ".x/.xm" file
* View - Inspector - File, select ".x" file in project navigator, under 'Identity and Type' change 'Type' to 'Objctive-C Source', reopen ".x" file. The same steps for ".xm" file.
* You can now click the left gutter in the Xcode editor to set breakpoints directly in your Logos source code.
...
### 强制Xcode将".x/.xm"文件以源码对待

* Build Phases - Compile Sources - 添加".x/.xm"文件
* View - Inspector - File, 选中".x"文件, 在'Identity and Type'设置'Type'为'Objctive-C Source', 关闭并重新打开".x"文件. ".xm"文件操作方式类似.
* 此时可以在Xcode中左侧下断点了
